;(function($) {
	var form = function($form, options) {
		...
		self.errorMessage = function (html, showTitle) {
			...
			if (showTitle !== false) {
			    $message.append($('<div class="error-title"/>').html(options.errorTitle));
			}
						...
						if (response.type === 'success') {
							...
							if (typeof options.success === 'function') {
								...
								var success = function () {
			                        ...
			                        var $successMessage = $('<div class="success-message"/>').html(response.message).hide().insertBefore(self.$container).fadeIn(options.successFadeInSpeed).show(0, function() {
		                                // successEnd function
								...
								$.each(response.elementErrors, function(index, info) {
									...
									if (typeof info.errors === 'object' && info.errors.length > 0) {
										...
										if ($element.length) {
											...
											var $errorList = $('<div class="errors cf"/>'),
											$errorWrap = $('<div class="errors-wrap"/>').append($errorList);

											// Append the error
											$errorList.append('<div class="error">' + info.errors[0] + '</div>');

											// Add the error list after the element's wrapper
											$element.parents('.input').append($errorWrap).end().parents('.element').addClass('has-error');
	...
	$.fn.form = function (options) {
		...
		return this.each(function () {
			var $this = $(this);
			if (!$this.data('form')) {
				$this.data('form', new form($(this), settings));
			}
		});
	}; // End $.fn.form
...
(function($) {
	$.fn.replaceSelectWithTextInput = function (options) {
		...
		return this.each(function () {
			var $this = $(this),
			$wrapper = $this.parent(),
			$element = $this.parents('.element').addClass('select-replaced'),
			html = $wrapper.html();
			...
			$wrapper.delegate('select', 'change', function () {
				if ($(this).val() == options.onValue) {
					...
					$element.removeClass('element-select').addClass('element-text');

					$cancel = $('<a>').click(function () {
						$wrapper.html(html);
						$element.removeClass('element-text').addClass('element-select');
						$(this).remove();
						return false;
					}).attr('href', '#').addClass('cancel-button').attr('title', options.cancel);
